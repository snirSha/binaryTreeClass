...
void Tree::insertPrivate(int key, node* ptr)
{
	...
	{
		throw std::invalid_argument("the key is already exist");
...
void Tree::removeRootMatch()
{
	if (_root != NULL)
	{
		...
		else
		{
			...
			cout << "The root ket containing " << rootKey << "was overwritten with key " << _root->key << endl;
		}
	}
	else
	{
...
int Tree::leftPrivate(int key,node* ptr)
{
    ...
    if(ptr==NULL)
    {
	throw std::invalid_argument("recieved negetive value");
